...
一个 Cursor "request" 从外部看是用户发送一条消息、Agent 执行一系列操作、返回结果。但内部实际上是**多次 LLM API 调用**的集合：

```
用户发送一条消息（= 1 个 "request"）
    │
    ▼
Cursor 构建系统提示
    │ +600~800 tokens 的隐藏系统提示
    │ +当前文件内容、项目元数据、OS 信息
    │ +对话历史
    │
    ▼
...
- 一个 "request" 内部可能包含多次 LLM API 调用
- 如果达到工具调用次数上限，需要继续对话（新的 request）
...
| 达到工具调用上限后继续 | **是** | 开始新 request，消耗额外额度 |
